...
> 本文档详细解析 Conv、MBConv、SPP、SPPF、C3、C2f、Concat 七个核心模块的底层架构、数学原理、设计动机及代码实现。
...
5. [C3（跨阶段局部网络-3卷积版）](#5-c3跨阶段局部网络-3卷积版)
6. [C2f（跨阶段局部网络-2卷积融合版）](#6-c2f跨阶段局部网络-2卷积融合版)
7. [Concat（拼接操作）](#7-concat拼接操作)
8. [模块继承关系总览](#8-模块继承关系总览)
...
```
    ...
    ┌───────┴───────────┐
    │                   │
 DepthwiseConv    普通Conv
    │                   │
 MBConv(倒残差)        │
 (EfficientNet)       C3 (CSP结构)
    │                   │
    │               ┌───┴───┐
    │               │       │
    │              C2f    SPP
